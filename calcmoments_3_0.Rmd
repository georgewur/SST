...
```{r}
sampler_rows = c(1)#c(20,25,30)
sampler_cols = c(1)#c(75,79)
```
...
delx = 2.0 #10.  dx=1.0 for test uniform k field
dely = 2.0 #10.
delz = 2.0 #10.
porosity =0.3
...
Trans_conc = readucn("Hetero_H_F_nl1",NLAY = 1, NTTS = 73)
...
for (i in 1:length(pts))#requires gifski and ellipse packages!!
{
  ...
  polygon(ellipse(M,scale=c(sqrt(M2_x_time[i]),sqrt(M2_y_time[i])),centre=c(M1_x_time[i],M1_y_time[i]),level=0.68,fill=TRUE),col=rgb(0,0,1,0.2),border=NA)
  grid(col="black")
}
